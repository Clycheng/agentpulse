...
| Hermes 集成 | 🟢 **主链已接入(2026-07-13)**：走 **ACP(stdio)**([ADR 0007](docs/decisions/0007-hermes-v0.18-interface-acp.md))——HermesBackend(TD-03-T2)→RunService(TD-03-T3)→自动供给(TD-03-T5)→**审批 suspend/resume+求援(TD-03-T4)**全链真机验证；员工还能**空闲攒 idea(TD-08-T2)+ 干活沉淀技能(TD-06-T1)**。开 `AGENTPULSE_HERMES_PROVISIONING=true`+DeepSeek key 即真员工执行。坑见 [ADR 0005](docs/decisions/0005-hermes-poc-safety-findings.md) | 剩 TD-06-T2 能力升级、远程部署运维、成长/进度前端 |
