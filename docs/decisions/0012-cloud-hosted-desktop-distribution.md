...
6. 生产默认 fail-close：JWT 签名密钥与凭证加密密钥必须独立且至少 32 字符；Hermes 只接收白名单环境变量并启用托管安全模式；公网 Caddy 不暴露 MCP，下载埋点只保存按日聚合计数。反向代理从固定 Caddy 源码提交使用已修复工具链构建，不能因为上游官方镜像可用就豁免 HIGH/CRITICAL 发布门。
...
- `auth_secret_key` 只签发 JWT，`credential_encryption_key` 单独派生版本化 Fernet key。两者都必须持久备份；凭证密钥轮换前需实现显式重加密。
- 免费层不能替代生产 SLA。镜像必须在每次发布前完成依赖审计和 HIGH/CRITICAL 漏洞扫描，发现有修复版本的高危项即阻止发布。
