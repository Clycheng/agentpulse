...
API 镜像以 uid 10001 非 root、只读根文件系统、drop all capabilities 和 `no-new-privileges` 运行。构建时应用 Debian 安全更新，并在不改变 Hermes 固定源码提交的前提下覆盖已知易受攻击的依赖约束。Caddy 不直接使用仍含已知漏洞的官方运行镜像：从固定 `v2.11.4` 提交构建，使用已修复的 Go 1.26.5、gRPC 1.82.1 和升级后的最小 Alpine 运行层。CI 运行 `pip-audit`、Bandit 和 Trivy，存在可修复 HIGH/CRITICAL 项即失败。公网 Caddy 阻断 `/mcp`，限制请求体和日志尺寸；宿主机只开放 SSH/HTTP/HTTPS。
...
限流记录持久化到数据库，IP 仅以服务端 HMAC 摘要保存。JWT 校验 issuer、audience、type、jti、iat 和 exp；生产拒绝默认/重复/过短密钥、HTTP 或通配 CORS、未开启托管 Hermes 安全模式以及 TD-09 前启用入站 webhook。Hermes 子进程环境从空白白名单组装，托管模式禁用 terminal、files、browser、web、code execution 和 computer use，只保留 AgentPulse 动态 MCP。
...
Vercel 以 `agentpulse.cc` 为主域，`www` 重定向到 apex。官网按平台推荐下载并始终展示其他平台、版本、校验值和未签名安装步骤；接入无 Cookie 的 Vercel Web Analytics。下载和安装帮助事件发送到 API 后只按 UTC 日期、事件类型累加，不保存原始 IP、UA 或单用户标识。

### 当前执行证据（2026-07-24）

- `Clycheng/agentpulse-releases` 已创建为独立公开空仓，未改变源码仓可见性。
- Vercel `agentpulse-web` 的 Web Analytics 已启用；官网代码已包含 Insights 脚本和匿名下载事件。
- API 全套 `355 passed / 9 skipped`，desktop lint/build 通过；API 与 Hermes 两个 Python 环境 `pip-audit` 均为 0，npm audit 为 0，Bandit HIGH 为 0。
- ARM64 API 与自建 Caddy 镜像已本机构建，Trivy 可修复 HIGH/CRITICAL 均为 0；API 以生产 fail-close、非 root、只读根文件系统、零 capabilities 配置通过 ready smoke，Hermes v0.18.2 与系统 libpq 均实测可用。
- macOS arm64 DMG/ZIP 已生成并通过 `hdiutil verify`/ZIP 完整性检查。
- Vercel 上误建的 `agent-pulse-service` 会在请求时崩溃，不作为 API 宿主；`api.agentpulse.cc` 必须在 Oracle 节点就绪后迁移，绝不把 FastAPI/Hermes 改造成 Vercel Function。
- Oracle、Supabase、最终 DNS/HTTPS、Windows CI 资产和线上 E2E 尚未完成，因此本 TD 保持进行中。
