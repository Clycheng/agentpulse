...
### TD-04-T6：LocalHermesProvisioner 真实现〔须 Hermes 验证会话〕
- 改动点：真实 `hermes profile create --no-alias`(实测过) + 写 SOUL.md(实测过) + `configure()`/`install_skills()`(**先实测确认语法**：per-profile `mcpServers`、toolsets 开关、技能安装；抄真名回填 DATA-MODEL 与 TD-05)。
