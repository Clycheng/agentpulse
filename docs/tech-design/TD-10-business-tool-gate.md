...
```
员工 SOUL 里配的能力 → 供给时 hermes mcp add 挂一个我们自己的 MCP 服务
                                    │
                agent 在推理里决定"发这条社媒内容" / "发这封邮件"
                 ...
                 我们的 MCP 服务（挂在 services/api 同一个 FastAPI 进程里）
                        POST /mcp/business-tools （HTTP-based MCP transport）
                                    │
                 1. 从请求头的 per-profile token 反查 agent_id/workspace_id
                 ...
                 3. risk_gate == 'auto'      → 直接执行，返回结果
                    ...
                    risk_gate == 'prohibited_auto' → 挂起，且永不因超时之外的
                                                      理由自动放行
                                    │
                       （挂起分支）写 approvals 行(type='business_tool')
...
- 挂在 `services/api` 同一个 FastAPI 进程下的新路由 `app/api/routes/mcp_business_tools.py`，用官方 `mcp` Python SDK（新依赖，`requirements.txt` 加 `mcp`）实现 **streamable-HTTP transport**（Hermes `hermes mcp add <name> --url <endpoint> --auth header --env MCP_TOKEN=<token>` 就是奔着这个协议去的，DATA-MODEL §5.3 已经验证过这条 CLI 命令真实存在）。
...
- 供给时（`LocalHermesProvisioner.configure`）：如果这个员工被授予的能力里有任何一个带 `business_tool`，额外跑一次 `hermes mcp add agentpulse-business --url {settings.public_base_url}/mcp/business-tools --auth header --env MCP_TOKEN={per_profile_token}`。`per_profile_token` 就是这个 agent 的 `agent_specs.hermes_profile` 值本身（已经是随机、按 workspace+agent 派生的字符串，够用；不新建鉴权表）——MCP 服务收到请求时按这个 token 反查 `agent_specs WHERE hermes_profile = ?` 拿到 `agent_id`/`workspace_id`。
- **定位 run_id（已知的技术限制，明确写清楚不是漏掉了）**：MCP 工具调用发生在跟 `stream_agent_run` 完全独立的请求里，没有天然的 run_id 可用。v1 做法：查 `SELECT id FROM runs WHERE agent_id = ? AND status IN ('running','waiting_user') ORDER BY created_at DESC LIMIT 1` 拿"这个员工当前这条正在跑的 run"，写进 `approvals.run_id`（供 RunTrace 审计用）。这是一个基于"单进程部署、同一员工同时只有一条活跃 run"的启发式假设（`approval_bridge.py` 现有文档已经承认了同样的单进程假设），多员工并发不受影响；同一员工被并发触发多条 run 的极端情况超出 v1 范围。
