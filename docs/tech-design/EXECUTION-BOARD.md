...
| 序 | 任务 | 一句话 | 会话要求 | 状态 |
|---|---|---|---|---|
| 1 | [TD-03-T4](TD-03-hermes-execution.md) | **审批 suspend/resume 闭环**：approval_required 现在是 deny-by-default（安全）；做成挂起 `runs.status=waiting_user`+建 approval → 老板批准 → 续跑同一 run（跨请求恢复）；+ clarification_required | **agentpulse**(起 Hermes) | ⚪ 待领 |
...
| 任务 | commit | 备注 |
|---|---|---|
| **TD-03-T4 审批 suspend/resume + clarification**：`runtime/approval_bridge.py`（进程内 Future 注册表，`await_decision`/`resolve_pending` 线程安全唤醒同一在飞 run）；HermesBackend `request_permission` 生成 `approval_id`、发 approval_required 事件、`await` resolver 原地挂起；run() 循环挂起期禁用超时；RunService 收到 approval_required→落 `approvals`(带 run_id+type)+转 `waiting_user`/`waiting_clarify`+commit，resolver=`make_bridge_resolver` 阻塞在桥上；热路径 `_stream_reply_events` 注入该 resolver；`/approvals/{id}/resolve` 对 run 关联审批→转 run→`resolve_pending(allow_once/deny)`（跳过旧的"审批即完成任务"），新增 `/approvals/{id}/answer`（clarification：记录答复为消息+续跑）。**ACP 是 stdio 原地挂起模型**（run 状态在子进程里，只能唤醒活协程，不能 detach 重连）。**零回归**：新增 9 单测（桥/挂起→approve 续跑/deny 走替代/resolve 端点转 run/answer 记录+续跑/非 clarification 拒绝）；全套 **237 通过 + 7 skipped**。三条 grep 干净 | 2026-07-13(见 CHANGELOG) | 北极星「老板拍板制」双向闭环打通；clarification 答复文本因 ACP 只传 allow/deny 暂经会话历史带回（真·inline 注入待 Hermes resume API） |
| **TD-06-T1 技能自动沉淀（Reflection）**：`runtime/reflection.py`——`_summarize_recent_steps`(拉该员工最近 N 个 completed run 的 run_steps) → `run_reflection`(注入提炼 prompt→经员工自己的 Hermes profile 执行→`parse_skills` 容错解析严格 JSON→`ProfileProvisioner.update_skill` 写进 profile `skills/auto/*.md`→重置 `runs_since_last_reflection`+stamp)；`bump_reflection_counter`(RunService 每完成一个 run +1，到 `reflection_interval` 触发) 接在 `runner.stream_agent_run` 完成路径；`run_reflection_tick` 接进后台 cron。`ProfileProvisioner` 加 `update_skill`/`list_skills`（RecordOnly + LocalHermes，中文名哈希避免文件名碰撞）；`agent_specs` 加 `runs_since_last_reflection`/`last_skill_reflection_at`/`reflection_interval` 三列（双 schema）；API `GET /api/agents/{id}/skills` + `POST /api/agents/{id}/reflect`；SOUL 注入"沉淀经验"指令。**真机验证**：LocalHermes 物理写入/读取 skills/auto 通过；reflection e2e(HERMES_E2E=1) 跑通。**零回归**：新增 14 单测；全套 **230 通过 + 7 skipped** | 2026-07-13(见 CHANGELOG) | 北极星④「越用越懂你」后端闭环；skill_learned run_step 因 run_steps.type CHECK 未含而略过（技能可见性走 /skills 读目录） |
