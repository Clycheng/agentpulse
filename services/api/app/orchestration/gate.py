...
def validate_task_creation_gate(
    ...
    *,
    ...
    Gate rules:
    1. If consensus_brief_id is provided:
       - Brief must exist in workspace
       - Brief must be in 'confirmed' status
    2. If consensus_brief_id is None AND parent_task_id is provided:
       - Allow (sub-task inherits parent's brief)
    3. If consensus_brief_id is None AND parent_task_id is None:
    ...
    if consensus_brief_id is None:
        ...
        # Sub-task inherits parent's brief - no validation needed
        return None
